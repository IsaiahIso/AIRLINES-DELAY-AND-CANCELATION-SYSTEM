...
if (isset($_POST['submit'])) {
    ...
    if ($request_type == 'delay') {
        ...
        if ($row = mysqli_fetch_assoc($result)) {
            ...
            $delay_hours = ($actual_departure_time - $departure_time) / 3600; // Convert seconds to hours

            if ($delay_hours >= 2) {
                $compensation_amount = 5000; // Example: 5,000 KES for 2+ hour delays
            }
            if ($delay_hours >= 5) {
                $compensation_amount = 10000; // 10,000 KES for 5+ hour delays
            }
        }
    }
