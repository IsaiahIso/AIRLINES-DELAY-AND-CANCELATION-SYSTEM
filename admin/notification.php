...
if ($passengerResult) {
    while ($row = mysqli_fetch_assoc($passengerResult)) {
        $passengerEmails[] = [
            'email' => $row['email'],
            'first_name' => $row['first_name'] ?? 'Valued Passenger' // Default name if null
        ];
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['notify'])) {
    if (empty($passengerEmails)) {
        ...
        foreach ($passengerEmails as $passenger) {
            ...
            $first_name = !empty($passenger['first_name']) ? htmlspecialchars($passenger['first_name'], ENT_QUOTES, 'UTF-8') : 'Valued Passenger';
            ...
            try {
                ...
                $mail->SMTPAuth = true;
                $mail->Username = 'lesorogol1@gmail.com';  // Your SMTP email
                $mail->Password = 'vuvt mwxq pgfy zmrx';  // Your SMTP password
                $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
                $mail->Port = 587;

                $mail->setFrom('lesorogol1@gmail.com', 'Flight Disruption Notifications');
...
<?php 
    if (!empty($emailStatus)) {
        if (array_filter($emailStatus, fn($msg) => strpos($msg, 'Error') !== false)) {
            echo "<p class='message error'>Some emails failed to send.</p>";
        } else {
            echo "<p class='message success'>All emails sent successfully.</p>";
        }
    }
    ?>
</div>
